...
import { AppUpdateDownloadStatus } from "@/models/data/states/AppUpdateDownloadState"

export default function UpdateVersionScreen() {
  const { translations } = useLanguageState()
  const { latestVersion, updateAppVersion, appUpdateDownloadState } = useGlobalState()
  ...
  const versionInfo: Version | null = latestVersion && latestVersion.version ? latestVersion : null
  const mapStatusForIndicator = (status: AppUpdateDownloadStatus): "downloading" | "completed" | "error" | "idle" => {
    switch (status) {
      case 'downloading':
        return 'downloading'
      case 'installing':
      case 'completed_relaunching':
        return 'completed'
      case 'error':
        return 'error'
      case 'checking':
  ...
  if (!versionInfo) {
    return (
      <div className="p-6 bg-card text-card-foreground rounded-lg shadow-md max-w-md mx-auto space-y-4 text-center">
        <p>{translations.loadingUpdateInfo}</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 bg-card text-card-foreground rounded-lg shadow-md max-w-md mx-auto space-y-4">
      <div className="text-center">
        <h2 className="text-xl font-bold text-primary">{translations.updateScreenTitle}</h2>
        <p className="text-xs text-muted-foreground">{translations.updateScreenSubtitle}</p>
      </div>

      {!appUpdateDownloadState.isActive ? (
        ...
        <div className="py-4">
          <p className="text-lg font-semibold text-center mb-3">{appUpdateDownloadState.message}</p>
          <DownloadProgressIndicator
            progress={appUpdateDownloadState.progress}
            status={mapStatusForIndicator(appUpdateDownloadState.status)}
            fileName={`LemBraGO ${versionInfo.version}`} // Usando a versão do objeto versionInfo
          />
          {appUpdateDownloadState.status === 'completed_relaunching' && (
            <p className="text-sm text-primary text-center mt-3">{translations.restartingAppMessage || "Reiniciando o aplicativo..."}</p>
          )}
        </div>
      )}
    </div>
